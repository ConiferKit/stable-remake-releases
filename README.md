...
Since rc.11, beta routing is cache-aware: each `beta-router` decision reports which candidate models still have this conversation in a warm prompt cache on the connection Stable would use for them (your Conifer API key, Claude subscription or ChatGPT subscription), so the router can price them accurately. The report contains only model IDs, token counts and cache ages, never prompt text. A gateway that doesn't support it gets the unchanged request.
